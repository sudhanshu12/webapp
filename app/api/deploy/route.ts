...
export async function POST(request: NextRequest) {
  try {
    const { platform, domain, accessToken, wizardData } = await request.json()

    if (!platform || !domain || !accessToken || !wizardData) {
      return NextResponse.json({ error: 'Missing required parameters' }, { status: 400 })
    }

    // Get user from session (you'll need to implement proper authentication)
    const userId = 'current_user_id' // This should come from your auth system
    ...
    // Generate static site files from wizard data
    const siteFiles = await generateStaticSite(wizardData, platform)

    // Deploy to platform using specific APIs
    let deploymentResult: any
    if (platform === 'vercel') {
      const vercelResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/deploy/vercel`, {
        method: 'POST',
        headers: {
      ...
      if (!vercelResponse.ok) {
        ...
        return NextResponse.json({ error: error.error || 'Vercel deployment failed' }, { status: 400 })
      }
    ...
    await supabase
      ...
      .insert({
        ...
        type: 'deduct',
        amount: 1,
        description: `Deployment to ${platform}: ${domain}`
      })
...
async function generateStaticSite(wizardData: any, platform: string) {
  // Generate HTML, CSS, and JS files based on wizard data
  ...
  return {
    ...
    'styles.css': css,
    'script.js': js,
    'package.json': generatePackageJson(platform)
  }
}
...
function generatePackageJson(platform: string) {
  if (platform === 'vercel') {
...
async function deployToNetlify(domain: string, files: any, accessToken: string) {
...
async function deployToVercel(domain: string, files: any, accessToken: string) {
  // In a real implementation, you would:
