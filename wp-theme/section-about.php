...
// Get domain name from settings, fallback to current domain
$domain_name = $settings['domain_name'] ?? '';
if (!empty($domain_name)) {
    // Ensure domain has protocol
    $domain_name = (strpos($domain_name, 'http') === 0) ? $domain_name : 'https://' . $domain_name;
    $about_cta_link = $domain_name . '/' . ($settings['about_button_link'] ?? 'about-us');
} else {
    // Fallback to current domain
    $about_cta_link = home_url('/' . ($settings['about_button_link'] ?? 'about-us'));
}
// Use about_home_image first, fallback to about_image, then default roofing placeholder
$about_image = !empty($settings['about_home_image']) ? $settings['about_home_image'] : 
    ...
    <div class="homepage-about-flex" style="display: flex; align-items: center; gap: 3rem; max-width: 1200px; margin: 0 auto; padding: 3rem 120px; justify-content: center;">
        <div class="homepage-about-img" style="flex: 1; display: flex; align-items: center; justify-content: center;">
            <?php if (!empty($about_image)): ?>
                <img src="<?php echo esc_url($about_image); ?>" alt="Our Team" style="max-width: 350px; width: 100%; border-radius: 8px; box-shadow: 0 2px 16px rgba(0,0,0,0.08);" class="lazy-load-image" data-src="<?php echo esc_url($about_image); ?>" loading="lazy" decoding="async">
            <?php else: ?>
