...
export async function POST(req: NextRequest) {
  try {
    ...
    const aboutDesc = data.about_home_desc || `With over 15 years of experience serving ${location}, we've built a reputation for excellence in ${businessType}.`;
    ...
    const locations = (data.service_areas || []).map((l: any) => ({
      name: l.name || '',
      state: l.state || ''
    }));
    ...
    const reviews = [
      {
        name: 'Sarah Johnson',
        text: `${businessName} transformed our project with their professional service.`,
        date: '2024-08-15'
      },
      {
        name: 'Mike Chen',
        text: `Outstanding work! ${businessName} completed our project on time.`,
        date: '2024-08-10'
      }
    ];
...
$services = array(${services.map((s: any) => `array('name' => '${escapePhpString(s.name)}', 'description' => '${escapePhpString(s.description)}')`).join(', ')});
$reviews = array(${reviews.map((r: any) => `array('name' => '${escapePhpString(r.name)}', 'text' => '${escapePhpString(r.text)}', 'date' => '${r.date}')`).join(', ')});
$service_areas = array(${locations.map((l: any) => `array('name' => '${escapePhpString(l.name)}', 'state' => '${escapePhpString(l.state)}')`).join(', ')});
...
*/`);
    zip.file('index.php', `<?php include 'home-dynamic.php'; ?>`);
...
`);
    ...
    console.log('=== PIXEL PERFECT THEME GENERATED ===');
